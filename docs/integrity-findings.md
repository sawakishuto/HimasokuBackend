...
> 補足: `AppDelegate.swift` はインデントに通常スペースと非標準の空白文字が混在しており、
> 機械的な編集が壊れやすい。この修正は Xcode 上でコンパイル確認しながら手で行うのが安全。

### 6. 通知送信が同期処理【パフォーマンス】
`NotificationService#deliver` はデバイストークンごとに APNS へ同期送信しており、
グループの人数分だけ HTTP リクエストの処理時間が伸びる（大規模グループでタイムアウト懸念）。
ActiveJob（本番用のキューアダプタ導入が前提）でバックグラウンド化するのが望ましい。
なお `device_tokens_for` の N+1 クエリは 1 クエリに最適化済み。
